...
# class UserAdminListViewTest(TestCommon):

#     def setUp(self):
#         super(UserAdminListViewTest, self).setUp()
#         self.user = User.objects.create_superuser(email='cfs@cfs.com', full_name='Christopher Sanders', password=self.pwd)

#     def test_correct_view_used(self):
#         self.login_test_user(self.user.email)
#         found = resolve(reverse('profiles:users_list'))
#         self.assertEqual(found.func.__name__, UserAdminListView.as_view().__name__)

#     def test_view_returns_correct_status_code(self):
#         self.login_test_user(self.user.email)
#         response = self.client.get(reverse('profiles:users_list'))
#         self.assertEqual(response.status_code, 200)
...
#         self.assertRedirects(
#             response,
#             expected_url='{}?next=/profiles/admin/users/'.format(reverse(settings.LOGIN_URL)),
#             status_code=302,
#             target_status_code=200,
#             msg_prefix=''
#         )

#     def test_view_non_superuser_access_denied(self):
#         user = User.objects.create_user(email='vero7@vero.com', full_name='Veronica Rodriguez', password=self.pwd)
#         self.login_test_user(user.email)
#         response = self.client.get(reverse('profiles:users_list'))
#         self.assertEqual(response.status_code, 403)
...
# class UserInviteViewTest(TestCommon):

#     def setUp(self):
#         super(UserInviteViewTest, self).setUp()
#         self.user_data = {
#             'email': 'cham@cham.com',
#         }
#         self.user = User.objects.create_superuser(email='cfs@cfs.com', full_name='Christopher Sanders', password=self.pwd)

#     def test_view_superuser_access(self):
#         user = User.objects.create_user(email='cham@cham.com', full_name='Cham Cham', password=self.pwd, is_superuser=False)
#         self.login_test_user(user.email)
#         response = self.client.get(reverse('profiles:user_invite'))
#         self.assertEqual(response.status_code, 403)
...
#     def test_correct_view_used(self):
#         self.login_test_user(self.user.email)
#         found = resolve(reverse('profiles:user_invite'))
#         self.assertEqual(found.func.__name__, UserInviteView.as_view().__name__)
...
#         response = self.client.post(reverse('profiles:user_invite'), data=self.user_data)
#         self.assertIsInstance(response.context['form'], InviteForm)

#     def test_invalid_data_shows_form_errors(self):
#         self.login_test_user(self.user.email)
#         self.user_data['email'] = ''
#         response = self.client.post(reverse('profiles:user_invite'), data=self.user_data)
#         self.assertContains(response, escape(_('validation_field_required')))

#     def test_email_sent_on_success(self):
#         self.login_test_user(self.user.email)
#         self.client.post(reverse('profiles:user_invite'), self.user_data)
#         self.assertEqual(len(mail.outbox), 1)
#         self.assertEqual(mail.outbox[0].to, [self.user_data['email'].lower()])

#     def test_email_not_sent_on_failure(self):
#         self.login_test_user(self.user.email)
#         self.user_data['email'] = 'cham'
#         self.client.post(reverse('profiles:user_invite'), self.user_data)
#         self.assertEqual(len(mail.outbox), 0)

#     def test_view_redirect_on_success_created_user(self):
#         self.login_test_user(self.user.email)
#         response = self.client.post(reverse('profiles:user_invite'), data=self.user_data)
#         self.assertRedirects(
#             response,
...
#     def test_view_redirect_on_success_existing_user(self):
#         self.login_test_user(self.user.email)
#         User.objects.create_user(email=self.user_data['email'], full_name='Cham Cham', password=self.pwd)
#         response = self.client.post(reverse('profiles:user_invite'), data=self.user_data)
#         self.assertRedirects(
#             response,
#             expected_url='/',
...
#         )
    ...
    # def test_view_context_data(self):
    #     # Test if validlink is false with altered uid.
    #     response = self.client.get(
    #         reverse('profiles:register_confirm', kwargs={'uidb64': 'asdf', 'token': self.token})
    #     )
    #     self.assertFalse(response.context['validlink'])
    #     # Test if validlink changes from true to false after successful registration.
    #     response = self.client.get(
    #         reverse('profiles:register_confirm', kwargs={'uidb64': self.uid, 'token': self.token})
    #     )
    ...
    # def test_view_redirect_on_success(self):
    #     response = self.client.post(
    #         reverse('profiles:register_confirm', kwargs={'uidb64': self.uid, 'token': self.token}),
    #         self.user_data
    #     )
    #     self.assertRedirects(
    #         response,
    #         expected_url=reverse('security:login'),
    #         status_code=302,
    #         target_status_code=200,
    #         msg_prefix=''
    #     )
...
# class UserDeleteViewTest(TestCommon):

#     def setUp(self):
#         super(UserDeleteViewTest, self).setUp()
#         self.user = User.objects.create_superuser(email='cfs@cfs.com', full_name='Christopher Sanders', password=self.pwd)
#         self.user_2 = User.objects.create_user(email='vero@vero.com', full_name='Veronica Rodriguez', password=self.pwd)

#     def test_correct_view_used(self):
#         self.login_test_user(self.user.email)
#         found = resolve(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertEqual(found.func.__name__, UserDeleteView.as_view().__name__)

#     def test_view_returns_correct_status_code(self):
#         self.login_test_user(self.user.email)
#         response = self.client.get(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertEqual(response.status_code, 200)

#     def test_view_renders_correct_template(self):
#         self.login_test_user(self.user.email)
#         response = self.client.get(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertTemplateUsed(response, 'admin/user_delete_confirm.html')

#     def test_view_non_authenticated_user_redirected_to_login(self):
#         response = self.client.get(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertRedirects(
#             response,
#             expected_url='{url}?next=/profiles/admin/user/{pk}/delete/'.format(
#                 url=reverse(settings.LOGIN_URL), pk=self.user_2.id
#             ),
#             status_code=302,
#             target_status_code=200,
#             msg_prefix=''
...
#     def test_view_deletes_post_with_post_request(self):
#         self.login_test_user(self.user.email)
#         user_id = self.user_2.id
#         self.assertTrue(User.objects.filter(pk=user_id).exists())
#         self.client.post(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertFalse(User.objects.filter(pk=user_id).exists())

#     def test_view_raises_error_with_invalid_pk(self):
#         self.login_test_user(self.user.email)
#         response = self.client.get(reverse('profiles:user_delete', kwargs={'pk': 4040}))
#         self.assertEqual(response.status_code, 404)
#         response = self.client.post(reverse('profiles:user_delete', kwargs={'pk': 4040}))
#         self.assertEqual(response.status_code, 404)

#     def test_view_redirects_on_success(self):
#         self.login_test_user(self.user.email)
#         response = self.client.post(reverse('profiles:user_delete', kwargs={'pk': self.user_2.id}))
#         self.assertRedirects(
#             response,
#             expected_url=reverse('profiles:users_list'),
#             status_code=302,
#             target_status_code=200,
#             msg_prefix=''
#         )
