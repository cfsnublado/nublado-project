...
from ..managers import VocabEntryManager, VocabSourceManager
from ..models import (
    CreatorModel, VocabSourceContentModel, VocabEntry, VocabContext, VocabContextEntry,
    VocabSource
)
...
class VocabSourceTest(TestCommon):
    ...
    def test_unique_together_creator_name(self):
        ...
        source_name = 'A reading test source'
        vocab_source_1 = VocabSource.objects.create(creator=self.user, name=source_name)
        vocab_source_1.full_clean()
        vocab_source_2 = VocabSource.objects.create(creator=user_2, name=source_name)
        vocab_source_2.full_clean()
        with self.assertRaises(IntegrityError):
            vocab_entry_3 = VocabSource.objects.create(creator=self.user, name=source_name)
