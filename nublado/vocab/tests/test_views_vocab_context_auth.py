...
from ..models import VocabContext, VocabProject, VocabSource
...
class VocabContextCreateViewTest(TestCommon):
    ...
    def test_view_non_authenticated_user_redirected_to_login(self):
        ...
        self.assertRedirects(
            response,
            expected_url='{0}?next=/{1}/source/{2}-{3}/context/create'.format(
    ...
    def test_view_renders_correct_template(self):
        ...
        self.assertTemplateUsed(response, '{0}/auth/vocab_context_create.html'.format(APP_NAME))
    ...
    def test_invalid_data_shows_form_errors_and_does_not_save(self):
        ...
        self.assertIsInstance(response.context['form'], VocabContextCreateForm)
        self.assertFormError(response, 'form', 'content', _('validation_field_required'))
    ...
    def test_view_redirects_on_success(self):
        ...
        self.assertRedirects(
            response,
            expected_url=reverse('vocab:vocab_context_tag', kwargs={'vocab_context_pk': vocab_context.id}),
            status_code=302,
            target_status_code=200,
            msg_prefix=''
        )
...
class VocabContextDeleteViewTest(TestCommon):
    ...
    def test_view_non_authenticated_user_redirected_to_login(self):
        response = self.client.get(
            reverse('vocab:vocab_context_delete', kwargs={'vocab_context_pk': self.vocab_context.id})
        )
        self.assertRedirects(
            response,
            expected_url='{0}?next=/{1}/context/{2}/delete/'.format(
                reverse(settings.LOGIN_URL),
                URL_PREFIX,
                self.vocab_context.id
            ),
            status_code=302,
            target_status_code=200,
            msg_prefix=''
    ...
    def test_view_renders_correct_template(self):
        ...
        self.assertTemplateUsed(response, '{0}/auth/vocab_context_delete_confirm.html'.format(APP_NAME))
    ...
    def test_view_deletes_object(self):
        ...
        self.client.post(
            reverse('vocab:vocab_context_delete', kwargs={'vocab_context_pk': obj_id})
        )
